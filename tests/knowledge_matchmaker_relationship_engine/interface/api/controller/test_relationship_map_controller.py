...
class TestRelationshipMapController:
    ...
    def test_should_return_relationships_in_response(self, client):
        response = client.post("/map", json={"draft": "My draft about cognition."})

        assert_that(response.json()["relationships"]).is_length(1)

    def test_should_not_return_pointers_key_in_response(self, client):
        response = client.post("/map", json={"draft": "My draft about cognition."})

        assert_that(response.json()).does_not_contain_key("pointers")

    def test_should_not_have_summary_field_in_pointer_dto(self):
        assert_that(PointerDto.model_fields).does_not_contain_key("summary")

    def test_should_not_have_content_field_in_pointer_dto(self):
        assert_that(PointerDto.model_fields).does_not_contain_key("content")

    def test_should_not_have_abstract_field_in_pointer_dto(self):
        assert_that(PointerDto.model_fields).does_not_contain_key("abstract")

    def test_should_not_have_text_field_in_pointer_dto(self):
        assert_that(PointerDto.model_fields).does_not_contain_key("text")

    def test_should_not_have_body_field_in_pointer_dto(self):
        assert_that(PointerDto.model_fields).does_not_contain_key("body")
